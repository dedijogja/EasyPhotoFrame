...
public class MultiDecryption extends AsyncTask<String, Void, List<Bitmap>>{
    ...
    public MultiDecryption(Context context) {
        this.context = context;
    }

    @Override
    protected void onPreExecute() {
        dialog = ProgressDialog.show(context, "Preparing frame",
                "Please wait ...", true);
        dialog.show();
    }
    ...
    protected List<Bitmap> doInBackground(String... params) {
        ...
        for (String aListFileDeskripsi : params) {
            try {
                InputStream inputStream = context.getAssets().open(Constant.FOLDER_KONTEN + "/" + aListFileDeskripsi);
                ...
                bitmaps.add(ThumbnailUtils.extractThumbnail(BitmapFactory.decodeByteArray(hasilDekripsi, 0, hasilDekripsi.length), 250, 150));
